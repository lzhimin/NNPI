...
class LeNet(PruningModule):
    ...
    def getInnerState(self, dataset):
        layer1_activation = []
        layer2_activation = []
...
class LeNet_5(PruningModule):
    ...
    def getInnerState(self,dataset):
        ...
        for i in range(len(dataset)):
            ...
            x = F.relu(x)
            x = F.max_pool2d(x, kernel_size=(2, 2), stride=2)
            conv1_activation.append(np.array(x.tolist()[0]).flatten().tolist())
            ...
            x = F.relu(x)
            x = F.max_pool2d(x, kernel_size=(2, 2), stride=2)
            conv2_activation.append(np.array(x.tolist()[0]).flatten().tolist())
