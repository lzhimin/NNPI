class SummaryView extends BasicView {
    ...
    draw_leaf(x, y, width, height, data, g){
        ...
        ratios.forEach((r)=>{
            ...
            Object.keys(r_m_info).forEach((m)=>{
                dataset[m].push(r_m_info[m]);
            });
        });

        Object.keys(dataset).forEach((m, i)=>{
            this.draw_leaf_helper(x + i * (width + padding) + width, y, width, height, dataset[m], g, m);
        });

        //setup the y-axis
        let y_axis = d3.scaleLinear().domain([0, 100]).range([y, y + height*0.9]);
        g.append('g')
    ...
    draw_leaf_helper(x, y, width, height, data, g, m){
        ...
        g.selectAll('.table_element')
            ...
            .attr('x', x)
            .attr('y', (d, i)=>{
                return y + i * height/10;
            })
            .attr('width', (d, i)=>{
                return  m =='accuracy'? d/100 * width : d * width;
            })
